    ...
    for epoch_num in range(parser.epochs):
        ...
        for iter_num, data in enumerate(dataloader_train):
            try:
                ...
                # Add the loss values to tensorboard
                writer.add_scalar('data/loss', loss, tot_iter)
                tot_iter += 1
        ...
        if parser.dataset == 'coco':
            ...
            for iter_num, data in enumerate(dataloader_val):
                ...
                with torch.no_grad():
                    ...
                    classification_loss, regression_loss = total_loss(classification, regression, anchors,
                                                                      data['annot'])
            ...
            print(
                'Validation epoch: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f} | Total loss: {:1.5f}'.format(
                    epoch_num, float(total_loss_classification), float(total_loss_regression), float(total_loss_joint)))

        scheduler.step(np.mean(epoch_loss))
        torch.save(retinanet, '{}_retinanet_{}_{}.pt'.format(parser.dataset, parser.depth, epoch_num))
